...
def generate_blog():
    ...
    try:
        ...
        if 'application/json' not in content_type:
            logger.error(f"Invalid response from API Gateway. Expected JSON, got '{content_type}'.")
            logger.error(f"Response text (first 500 chars): {response.text[:500]}")
            return jsonify({
                "error": "The content generation service returned an unexpected response. Please check the server logs."
            }), 502
        ...
        if s3_key and not content:
            ...
            try:
                ...
                # This is a likely source of the 500 error if permissions are wrong
        ...
        if user_id:
            # ✅ FIX: Use the globally defined in_memory_history dictionary
            ...
            if blogs_table:
                try:
                    dynamo_item = {
                        ...
                        'quality_score': str(history_item['quality_score']), # Storing as string is okay
                        'created_at': history_item['created_at'],
                        's3_key': history_item.get('s3_key')
                    }
                    # Remove keys with None values before saving to DynamoDB
    ...
    if blogs_table:
        try:
            ...
            if 'ResourceNotFoundException' in str(e):
                logger.error(f"DynamoDB GSI 'user_id-index' not found on table '{DYNAMODB_BLOGS_TABLE_NAME}'")
                return jsonify({"error": "History service configuration error. Index missing."}), 500
            logger.error(f"Error fetching history from DynamoDB: {e.response['Error']['Message']}")
            return jsonify({"error": "Failed to retrieve history due to a database error."}), 500
    else:
        user_history = in_memory_history.get(user_id, [])
        logger.info(f"Fetched {len(user_history)} history items from in-memory storage for user {user_id}")
